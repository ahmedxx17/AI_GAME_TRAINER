...
class FlappyBirdEnv:
    ...
    SCREEN_W = 400
    SCREEN_H = 600
    GRAVITY = 0.5         # Balanced gravity for natural oscillation
    FLAP_STR = -5         # Gentle flap — bird can make fine adjustments
    PIPE_SPEED = 2        # Slow pipes — more reaction time
    PIPE_GAP = 220        # Very wide gap — easy for agent to learn
    PIPE_WIDTH = 50       # Narrower pipes
    BIRD_X = 60           # Bird x-position
    BIRD_RADIUS = 12      # Smaller hitbox = more forgiving
    GAP_MARGIN = 60       # Keeps gaps away from screen edges
    ...
    def reset(self) -> np.ndarray:
        ...
        for i in range(pipe_count):
            self.pipes.append(
                Pipe(
                    # First pipe at x=150 — reaches bird around frame 45
                    x=float(150 + i * PIPE_SPACING),
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool]:
        ...
        if action == 1:
            self.bird_vel = self.FLAP_STR  # Standard override: single flap sets upward velocity
